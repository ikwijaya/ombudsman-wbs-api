...
module.exports = {
  ...
  async stepper(sid = null, id = null) {
    try {
       ...
       * 300 => Rollback Procedure
       */
      let r = await core.checkRoles(sessions[0].user_id, [1, 2, 3, 4, 5, 6, RollbackProcedure]);
      ...
      let complaint = await models.complaints.findAll(
        {
          attributes: [
            ...
            [Sequelize.literal(`case 
              ...
              when status.code='5' and complaint_actions.idx_t_complaint_action is not null then 'Tindak Lanjut oleh Inspektorat'
            ...
            [Sequelize.literal(`case 
              ...
              when status.code='5' and complaint_actions.idx_t_complaint_action is not null then 'grey lighten-2'
      ...
      let v = await models.complaints.findOne(
        {
          ...
          where: {
            ...
            [Op.and]: [
              Sequelize.literal(`DATE_PART('Year',dcreate)=DATE_PART('Year',now())`)
            ]
          },
          order: [
            ['dcreate', 'DESC']
          ], group: ['form_no', 'dcreate']
        }
      )
